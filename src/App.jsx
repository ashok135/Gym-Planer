...
function SyncDebugPanel({ user, syncError, isSyncing, lastSyncedTime, DB, META, FOOD, BUDGET, saveToFirestore }) {
  const [isOpen, setIsOpen] = useState(false);
  const [forceSyncMessage, setForceSyncMessage] = useState('');

  const handleManualSync = async () => {
    setForceSyncMessage('Syncing...');
    try {
      await saveToFirestore();
      setForceSyncMessage('Sync Completed successfully! ✅');
      setTimeout(() => setForceSyncMessage(''), 3000);
    } catch (err) {
      setForceSyncMessage(`Sync Failed ❌: ${err.message || String(err)}`);
    }
  };

  // Inspect data for June 7 - 9
  const targetDates = ['2026-06-07', '2026-06-08', '2026-06-09'];
  const localDataInfo = targetDates.map(date => {
    // Workout details
    const workout = DB[date];
    const workoutExercises = workout 
      ? Object.keys(workout)
          .filter(k => !['meta', 'customName', 'done'].includes(k))
          .map(k => workout[k].customName || k)
      : [];
    const workoutStatus = META[date]?.status || '';

    // Food/Nutrition details
    const food = FOOD[date];
    const hasFoodItems = food && food.items && Object.keys(food.items).length > 0;
    const foodWater = food?.water ? '💧' : '';
    const foodSleep = food?.sleep ? '😴' : '';
    const foodJunk = food?.junk ? '🚫 Junk' : '';

    // Budget details (Monthly structure - mk is YYYY-MM)
    const [y, m] = date.split('-');
    const mk = `${y}-${m}`;
    const budgetMonth = BUDGET[mk] || {};
    const budgetEntries = (budgetMonth.entries || []).filter(e => e.date === date);
    const extraIncome = (budgetMonth.extraIncome || []).filter(e => e.date === date);

    return {
      date,
      hasWorkout: workoutExercises.length > 0 || !!workoutStatus,
      workoutExercises,
      workoutStatus,
      hasFood: hasFoodItems || !!foodWater || !!foodSleep || !!foodJunk,
      foodHabits: [foodWater, foodSleep, foodJunk].filter(Boolean).join(' '),
      budgetSpent: budgetEntries.reduce((sum, e) => sum + Number(e.amount), 0),
      budgetEarned: extraIncome.reduce((sum, i) => sum + Number(i.amount), 0),
      budgetCount: budgetEntries.length + extraIncome.length,
      budgetNotes: [
        ...budgetEntries.map(e => `-${e.amount} (${e.note || e.category})`),
        ...extraIncome.map(i => `+${i.amount} (${i.label || 'Income'})`)
      ]
    };
  });
    ...
    <>
      ...
      <button 
        ...
        style={{
          ...
          border: `1px solid ${syncError ? 'var(--red)' : 'var(--accent)'}`,
          color: syncError ? 'var(--red)' : 'var(--text)',
          borderRadius: '50px',
          padding: '10px 16px',
          fontSize: '11px',
          fontWeight: 700,
      ...
      >
        <span>🛠️ Sync &amp; Data Status</span>
        {syncError ? (
          <span style={{ color: 'var(--red)', animation: 'pulse 1.5s infinite' }}>⚠️ Error</span>
        ) : isSyncing ? (
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <div style={{ fontSize: '18px', fontWeight: 800 }}>🛠️ Database Inspector</div>
              <button onClick={() => setIsOpen(false)} style={{ background: 'transparent', border: 'none', color: 'var(--text3)', fontSize: '20px', cursor: 'pointer' }}>✕</button>
            </div>

            {/* Sync Status Section */}
            <div style={{ background: 'var(--bg3)', padding: '16px', borderRadius: '16px', border: '1px solid var(--border2)', marginBottom: '20px' }}>
              <div style={{ fontSize: '14px', fontWeight: 700, marginBottom: '10px' }}>Cloud Sync Status</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: '12px', color: 'var(--text2)' }}>
                <div><strong>Logged In as:</strong> {user?.email}</div>
                <div><strong>Firebase UID:</strong> {user?.uid}</div>
                <div>
                  <strong>Sync Connection:</strong>{' '}
                  {syncError ? (
                    <span style={{ color: 'var(--red)', fontWeight: 700 }}>⚠️ Failed</span>
                  ) : (
                    <span style={{ color: 'var(--accent)', fontWeight: 700 }}>✅ Connected &amp; Synced</span>
                  )}
                </div>
                {lastSyncedTime && <div><strong>Last Synced At:</strong> {lastSyncedTime}</div>}
                {syncError && (
                  <div style={{ color: 'var(--red)', marginTop: '8px', background: 'rgba(239, 68, 68, 0.1)', padding: '8px', borderRadius: '8px', border: '1px solid rgba(239, 68, 68, 0.2)', fontSize: '11px', wordBreak: 'break-all' }}>
                    <strong>Error Message:</strong> {syncError}
                  </div>
                )}
              </div>
              ...
              {forceSyncMessage && (
                <div style={{ fontSize: '11px', textAlign: 'center', marginTop: '8px', color: 'var(--accent)', fontWeight: 600 }}>
                  {forceSyncMessage}
                </div>
              )}
            </div>

            {/* June 7 - June 9 Local Data Checker */}
            <div style={{ marginBottom: '20px' }}>
              <div style={{ fontSize: '14px', fontWeight: 700, marginBottom: '12px' }}>June 7 - June 9 Local Data Check</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {localDataInfo.map(info => (
                  <div key={info.date} style={{ background: 'var(--bg3)', padding: '14px', borderRadius: '16px', border: '1px solid var(--border2)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                      <span style={{ fontSize: '13px', fontWeight: 800, color: 'var(--accent)' }}>{info.date}</span>
                      <span style={{ fontSize: '11px', color: 'var(--text3)' }}>
                        {info.hasWorkout || info.hasFood || info.budgetCount > 0 ? '📝 Has Local Data' : '💨 Empty'}
                      </span>
                    </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: '12px', color: 'var(--text2)' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '6px' }}>
                        ...
                        <div>
                          ...
                          {info.hasWorkout ? (
                            <span>
                              {info.workoutStatus || 'Active'}{' '}
                              <span style={{ color: 'var(--text3)' }}>
                                ({info.workoutExercises.length} exercises logged)
                              </span>
                            </span>
                          ) : (
                            <span style={{ color: 'var(--text3)' }}>No workout logged</span>
                          )}
                          {info.workoutExercises.length > 0 && (
                            <div style={{ fontSize: '10px', color: 'var(--text3)', marginTop: '2px' }}>
                              {info.workoutExercises.join(', ')}
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        ...
                        <div>
                          ...
                          {info.hasFood ? (
                            <span>Logged {info.foodHabits}</span>
                          ) : (
                            <span style={{ color: 'var(--text3)' }}>No diet/habits logged</span>
                          )}
                        </div>
                      </div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '6px' }}>
                        ...
                        <div>
                          ...
                          {info.budgetCount > 0 ? (
                            <span>
                              Spent: <span style={{ color: 'var(--red)' }}>₹{info.budgetSpent}</span>, Earned: <span style={{ color: 'var(--accent)' }}>₹{info.budgetEarned}</span>
                            </span>
                          ) : (
                            <span style={{ color: 'var(--text3)' }}>No budget entries logged</span>
                          )}
                          {info.budgetNotes.length > 0 && (
                            <div style={{ fontSize: '10px', color: 'var(--text3)', marginTop: '2px', display: 'flex', flexDirection: 'column', gap: '2px' }}>
                              {info.budgetNotes.map((n, i) => <span key={i}>{n}</span>)}
                            </div>
                          )}
                        </div>
                      </div>
            ...
            <div style={{ fontSize: '11px', color: 'var(--text3)', textAlign: 'center', lineHeight: '1.4' }}>
              💡 If your local data is correct but not showing on other devices, click <strong>Force Sync to Cloud</strong>.
              If you see "Failed" under Sync Connection, check your internet or Firebase console security rules.
            </div>
